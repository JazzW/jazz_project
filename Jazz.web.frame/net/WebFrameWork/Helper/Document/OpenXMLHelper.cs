...
namespace WebFrameWork.Helper
{
    ...
    public class OpenXML
    {
        ...
        public void OpenDoc(string fileName,string filePath)
        {
            _filepath = Path.Combine(filePath, Guid.NewGuid().ToString() + ".docx");

            File.Copy(fileName, _filepath);
        ...
        {
            _wordProcDoc.Close();
            if (!_save && _filepath!=null)
                File.Delete(_filepath);
        }

        public void InsertText(string bookmark, string txt)
        {
            try
            {
                var bm = _mainDocPart.RootElement.Descendants<Word.BookmarkStart>().Where(e => e.Name == bookmark).FirstOrDefault().Parent;
                Word.Text text = bm.Descendants<Word.Text>().Where(e => e.Text == "_").FirstOrDefault();
                if (text == null)
                {
                    text = new Word.Text(txt);
                    //Word.Run run = new Word.Run(new Word.RunProperties(new Word.Bold()));
                    Word.Run run = new Word.Run();
                    run.Append(text);
                    Word.Paragraph par = new Word.Paragraph(run);
                    bm.InsertAfterSelf(par);
        ...
        public void InsertTable(string bookmark, Word.Table tb)
        {
            var bm = _mainDocPart.RootElement.Descendants<Word.BookmarkStart>().Where(e => e.Name == bookmark).FirstOrDefault().Parent;
            Word.Text text = bm.Descendants<Word.Text>().FirstOrDefault();
            if (text != null)
        ...
        public void InsertChart(string bookmark, Chart.Chart chart)
        {
            var bm = _mainDocPart.RootElement.Descendants<Word.BookmarkStart>().Where(e => e.Name == bookmark).FirstOrDefault().Parent;
            if (chart == null)
            {
                Pack.ChartPart[] parts = _mainDocPart.ChartParts.ToArray();
                Chart.ScatterChart sc = parts[0].ChartSpace.Descendants<Chart.ScatterChart>().First();
                Word.Text text1 = bm.Descendants<Word.Text>().FirstOrDefault();
                if (text1 != null)
                {
                    text1.Text = "";
                }
                bm.InsertAfterSelf<Chart.ScatterChart>(sc);
                return;
            }
            Word.Text text = bm.Descendants<Word.Text>().FirstOrDefault();
            if (text != null)
            ...
            bm.InsertAfterSelf<Word.Paragraph>(new Word.Paragraph(new Word.Run(chart)));
        }
        ...
        public void UpdateTable(DataTable dt)
        {
            ...
            {
                int maxColNum = dt.Columns.Count;
                Word.Table tb = _mainDocPart.RootElement.Descendants<Word.Table>().FirstOrDefault();
        ...
        public void UpdateTable(DataTable dt, int tbIndex)
        {
            try
            {
                int maxColNum = dt.Columns.Count;
